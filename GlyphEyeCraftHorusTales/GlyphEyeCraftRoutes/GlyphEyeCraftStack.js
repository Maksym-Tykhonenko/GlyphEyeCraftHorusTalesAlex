...
import ProductScreen from '../GlyphEyeCraftScreens/ProductScreen';
...
const GlyphEyeCraftStack = () => {
  ...
  OneSignal.initialize('274297cf-1113-4d30-930e-1083951f156a');
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2026-01-15T08:08:00'); //дата з якої поч працювати webView
  ...
  const Route = ({ isFatch }) => {
    ...
    return (
      <Stack.Navigator screenOptions={{ headerShown: false }}>
        ...
        <Stack.Screen
          name="GlyphEyeCraftAbout"
          component={GlyphEyeCraftAbout}
        />
        <Stack.Screen
  ...
  return (
    <>{!isLoading ? <GlyphEyeCraftLoader /> : <Route isFatch={route} />}</>
  );
};
